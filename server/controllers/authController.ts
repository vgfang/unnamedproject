...
export const registerViaEmail = async (
  ...
  try {
    ...
    if (registerRes.success) {
      ...
      // TODO: leaning towards just making register log the user in
...
export const loginViaEmail = async (
  ...
  try {
    const { email, password } = req.body;
    const loginRes = await AuthService.loginUserViaEmail(email, password);

    if (loginRes.success) {
      // set the refresh token in an http only cookie
      res.cookie("refreshToken", loginRes.jwtRefresh, {
        httpOnly: true, // prevents JavaScript access
        secure: process.env.NODE_ENV === "production", // use secure cookies in production
        maxAge: 30 * 24 * 60 * 60 * 1000, // 30 days in milliseconds
      });
      ...
      res.status(200).json({
        ...
        jwtAccess: loginRes.jwtAccess,
      });
    } else if (loginRes.hasOwnProperty("error")) {
      res.status(400).json({ success: false, message: loginRes.error });
    } else {
      res.status(500).json({ success: false, message: "unspecified" });
    }
    return;
